...
<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-6 font-title no-print">
        <div class="flex items-center gap-6">
            ...
            <!-- Title -->
            <h1 class="text-xl sm:text-2xl font-bold text-body-text hidden sm:block">
                Detail Absensi
            </h1>
        </div>
        ...
        <div class="flex gap-3 items-center">
            ...
            <form action="{{ route('rekap.show', $anggota['id']) }}" method="GET" class="flex gap-2">
                <input type="month" name="month" value="{{ $month }}" class="border border-gray-300 rounded-xl px-4 py-2 font-body text-sm" onchange="this.form.submit()">
            </form>
            <!-- Print Button -->
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 sm:p-8 flex flex-col md:flex-row gap-8 items-start md:items-center justify-between border border-gray-100">
        ...
        <div class="flex gap-4 sm:gap-6 w-full md:w-auto mt-4 md:mt-0 font-title">
            ...
            <div class="bg-[#2DA635]/10 border border-[#7AC77F] rounded-2xl p-4 flex-1 md:w-32 text-center">
                ...
                <p class="text-xs sm:text-sm font-semibold text-gray-600 mt-1 uppercase tracking-wider">Hadir</p>
            </div>
            <!-- Tidak Hadir -->
            <div class="bg-[#D91E2E]/10 border border-[#EC7E8B] rounded-2xl p-4 flex-1 md:w-32 text-center">
                ...
                <p class="text-xs sm:text-sm font-semibold text-gray-600 mt-1 uppercase tracking-wider">Tidak Hadir</p>
            </div>
            <!-- Total -->
            <div class="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex-1 md:w-32 text-center no-print">
                <p class="text-3xl sm:text-4xl font-bold text-blue-600">{{ count($jadwal) }}</p>
                <p class="text-xs sm:text-sm font-semibold text-gray-600 mt-1 uppercase tracking-wider">Jadwal</p>
            </div>
        </div>
    </div>

    <!-- Detail Table -->
    <div class="bg-white rounded-2xl shadow-sm p-4 sm:p-8 border border-gray-100">
        <h3 class="text-xl font-bold font-title text-body-text mb-6">Jadwal Kegiatan</h3>
        
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b-2 border-gray-200 font-title text-gray-700">
                ...
                <tbody class="font-body text-gray-600 divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="p-3 text-center">
                            @if($row['status'] === 'Hadir')
                                <span class="inline-block bg-[#2DA635]/20 text-[#238529] font-bold px-3 py-1 rounded-lg text-xs sm:text-sm border border-[#7AC77F]">
                                    HADIR
                                </span>
                            @else
                                <span class="inline-block bg-[#D91E2E]/20 text-[#D91E2E] font-bold px-3 py-1 rounded-lg text-xs sm:text-sm border border-[#EC7E8B]">
                                    TIDAK HADIR
                                </span>
                            @endif
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="p-6 text-center text-gray-500 font-medium bg-red-50">
                            Tidak ada jadwal untuk bulan ini ({{ $month }}).<br>
                            Peran (Role) Anggota ini: <b>{{ $anggota['role'] }}</b>.<br>
                            Pastikan di tab <b>jadwal</b> Google Sheet, terdapat data pada kolom `role` yang berisi `{{ $anggota['role'] }}` dan tanggal yang sesuai dengan `{{ $month }}`.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
