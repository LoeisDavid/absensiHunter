...
class GoogleSheetsService
{
    ...
    /** Access token di-cache 55 menit — hanya 1 request per jam */
     ...
     * Mengembalikan array of 2D arrays (satu per range).
     *
     * Contoh: batchGet(['absensi!B:B', 'absensi!D:D'])
     * → [$colB, $colD] masing-masing array 2D.
     */
    public function batchGet(array $ranges): array
    {
        if (empty($ranges)) return [];
        $token    = $this->getAccessToken();
        $query    = implode('&', array_map(fn($r) => 'ranges=' . urlencode($r), $ranges));
        $response = Http::timeout(15)->withToken($token)
            ->get("{$this->baseUrl}/{$this->spreadsheetId}/values:batchGet?{$query}");
        if (!$response->successful()) return array_fill(0, count($ranges), []);
        $result = [];
        foreach ($response->json('valueRanges', []) as $i => $item) {
            $result[$i] = $item['values'] ?? []; // 2D array per range
        }
        return $result;
    }

    /**
     * BATCH GET khusus baris tunggal: untuk JOIN anggota by ID.
     * Input: array range string tiap baris (mis. ['anggota!A2:F2', 'anggota!A5:F5'])
     * Output: array of 1D row arrays.
     */
    public function batchGetRows(array $rowRanges): array
    {
        if (empty($rowRanges)) return [];
        $results = $this->batchGet($rowRanges);
        return array_map(fn($r) => $r[0] ?? [], $results);
    }
     ...
     * HANYA load kolom itu (1 API call) → lebih hemat.
     * Returns 1-indexed row number, 0 jika tidak ketemu.
     */
    ...
    /**
     * Ambil satu baris spesifik by row number (1-indexed). O(1).
     */
    ...
    /** Hitung baris data (excludes header) */
    ...
    /** Hapus cache Google Sheets token */
